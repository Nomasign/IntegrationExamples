...
| Duplicate events received | NomaSign retried (your endpoint was slow or returned non-2xx briefly) | Make your handler idempotent — use the event `id` (or `session.id` + `type`) as a deduplication key |
